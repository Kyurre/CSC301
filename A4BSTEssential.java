...
public class A4BSTEssential<K extends Comparable<? super K>, V> {
	...
	// Count the number of nodes with given Value
	...
	private int countValue(Node node, V value) {
		if (node == null) return sum;
		
		// access left subtree
		countValue(node.left, value);
		
		// check to see if the node value equals given value
